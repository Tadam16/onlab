...
class Vessel12Dataset(torch.utils.data.Dataset):
    ...
    def evalitem(self, idx):
        resimg = self.img[idx, :, :].reshape([1,512,512])
        resautoimg = self.autoimg[idx, :, :]
        resimg2 = np.concatenate((resimg, resautoimg.reshape([1,512,512])), axis=0).reshape([1, 2, 512, 512])
        resmask = self.mask[idx, :, :].reshape([1, 1, 512, 512])
        return torch.from_numpy(resimg2).float(), resimg[0, :, :], resautoimg, resmask
